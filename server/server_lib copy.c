...
void process_ssl(SSL* ssl)
{
    ...
    char szHttpServerResponse[MAX_PAYLOAD + 1] = {0};

    bytes = SSL_read(ssl, szPayload, sizeof(szPayload)); /* get request */
    szPayload[bytes] = 0;

    memset(szPayload, 65, g_payload - 75);
...
void process_request(SSL_CTX* ctx, int client)
{
    SSL *ssl;
...
int preprocess_request(void* args)
{
    ...
    SSL_CTX* ctx = _args->ctx;
    int client = _args->client;

    process_request(ctx, client);
}

void process_async_request(SSL_CTX* ctx, int client)
{
    ASYNC_JOB *job = NULL;
...
void run_async_mode(SSL_CTX *ctx, int server, const char* pem_public_file, const char* pem_private_file)
{
    ...
    {      
        ...
        process_async_request(ctx, client);
        close(client);
